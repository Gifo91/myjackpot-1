...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-a5806fcb";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-a5806fcb">'+'<span class="tile-face-a5806fcb tile-front-a5806fcb"></span>'+'<span class="tile-face-a5806fcb tile-back-a5806fcb"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a5806fcb")){return;}
var back=tile.querySelector(".tile-back-a5806fcb");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-a5806fcb");if(isMine){tile.classList.add("boom-a5806fcb");}
if(dim){tile.classList.add("dimmed-a5806fcb");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-a5806fcb")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-a5806fcb");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-a5806fcb")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-a5806fcb");void panelEl.offsetWidth;panelEl.classList.add("jolt-a5806fcb");}
endRound();}
